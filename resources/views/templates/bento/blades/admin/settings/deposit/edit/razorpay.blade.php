        ...
        <div class="flex flex-col lg:flex-row min-h-[calc(100vh-160px)]">
            ...
            <div class="flex-1 flex flex-col pt-8 lg:pl-16 overflow-y-auto custom-scrollbar" id="contentScrollContainer">
                <div class="max-w-3xl">
                    ...
                    <form id="razorpay-form" action="{{ route('admin.settings.deposit.razorpay.update') }}" method="POST"
                        class="space-y-12 pb-24" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="id" value="{{ $gateway->id }}">

                        @include('templates.' . $template . '.blades.admin.settings.deposit.edit.partials.razorpay_guide')
                        ...
                        <div class="settings-section">
                            ...
                            <div class="grid grid-cols-1 gap-x-12 gap-y-10">
                                <div class="flex flex-col gap-3">
                                    ...
                                    <div class="relative">
                                        <input type="password" name="key_id"
                                            value="{{ sandBoxCredentials(safeDecrypt(config('site.razorpay.key_id'))) }}"
                                    ...
                                    <span class="text-[10px] text-slate-600 font-medium italic">
                                        {{ __('Retrieve this from your Razorpay Dashboard->Settings->API Keys.') }}
                                    </span>
                                </div>
                                ...
                                <div class="flex flex-col gap-3">
                                    ...
                                    <div class="relative">
                                        <input type="password" name="key_secret"
                                            value="{{ sandBoxCredentials(safeDecrypt(config('site.razorpay.key_secret'))) }}"
                                    ...
                                    <span class="text-[10px] text-slate-600 font-medium italic">
                                        {{ __('Keep this secret. It is used to authenticate requests to Razorpay.') }}
                                    </span>
                                </div>
                                ...
                                <div class="flex flex-col gap-3">
                                    ...
                                    <span class="text-[10px] text-slate-600 font-medium italic">
                                        {{ __('The currency that will be used for Razorpay transactions.') }}
                                    </span>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="flex flex-col gap-3">
                                        <p class="text-xs text-slate-400 leading-relaxed font-medium">
                                            {{ __('To ensure real-time transaction updates, please add the following URL to your Razorpay Dashboard webhooks settings, selecting the "order.paid" event:') }}
                                        </p>
                                        <div class="relative group">
                                            <input type="text" readonly
                                                value="{{ route('api.v1.webhooks.razorpay.deposit') }}" id="webhook-url"
                        ...
                        <div class="pt-10">
                            <button type="submit" id="submit-btn"
                                ...
                                <span class="btn-text">{{ __('Save Razorpay Settings') }}</span>
                            </button>
                        </div>
                    </form>
                </div>
