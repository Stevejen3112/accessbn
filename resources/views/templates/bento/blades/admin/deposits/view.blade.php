        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-secondary border border-white/5 rounded-2xl p-6 relative">
                    ...
                    @php
                        $paystack_data = null;
                        if ($deposit->paymentMethod && str_starts_with($deposit->paymentMethod->pay, 'paystack')) {
                            $raw_dump = $deposit->getAttributes()['auto_res_dump'] ?? null;
                            if (is_string($raw_dump)) {
                                $paystack_data = json_decode($raw_dump, true);
                            } elseif (is_array($raw_dump)) {
                                $paystack_data = $raw_dump;
                            }
                        }
                    @endphp

                    @if ($paystack_data)
                        <div class="pt-6 border-t border-white/5 relative z-10 mb-6">
                            <h4 class="text-sm font-bold text-white mb-4 uppercase tracking-widest opacity-50">
                                {{ __('Paystack Transaction Details') }}</h4>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @if (isset($paystack_data['reference']))
                                    <div class="space-y-1.5 p-4 bg-black/20 rounded-xl border border-white/5">
                                        <p class="text-[10px] text-text-secondary uppercase tracking-widest font-bold">
                                            {{ __('Paystack Reference') }}</p>
                                        <div class="flex items-center justify-between gap-2">
                                            <p class="text-white font-mono text-sm truncate">
                                                {{ $paystack_data['reference'] }}</p>
                                            <button
                                                class="p-1.5 hover:bg-white/10 rounded-lg text-text-secondary transition-colors copy-btn cursor-pointer"
                                ...
                                @if (isset($paystack_data['channel']))
                                    <div class="space-y-1.5 p-4 bg-black/20 rounded-xl border border-white/5">
                                        <p class="text-[10px] text-text-secondary uppercase tracking-widest font-bold">
                                            {{ __('Payment Channel') }}</p>
                                        <p class="text-white font-bold text-sm">
                                            {{ ucfirst(str_replace('_', ' ', $paystack_data['channel'])) }}</p>
                                    </div>
                                @endif
                                    ...
                                    @if ($paystack_data['channel'] === 'card')
                                        <div class="space-y-1.5 p-4 bg-black/20 rounded-xl border border-white/5">
                                            <p class="text-[10px] text-text-secondary uppercase tracking-widest font-bold">
                                                {{ __('Card Details') }}</p>
                                            <p class="text-white font-bold text-sm">
                                                {{ $auth['brand'] ?? '' }} •••• {{ $auth['last4'] ?? '' }}
                                            </p>
                                            <p class="text-[10px] text-slate-500 font-medium">
                                                {{ $auth['bank'] ?? '' }} ({{ $auth['country_code'] ?? '' }})
                                            </p>
                                        </div>
                                ...
                                @if (isset($paystack_data['gateway_response']))
                                    <div class="space-y-1.5 p-4 bg-black/20 rounded-xl border border-white/5">
                                        <p class="text-[10px] text-text-secondary uppercase tracking-widest font-bold">
                                            {{ __('Gateway Response') }}</p>
                                        <p class="text-white font-bold text-sm">{{ $paystack_data['gateway_response'] }}
                                        </p>
                                    </div>
                                @endif
                    ...
                    @php
                        $razorpay_data = null;
                        if ($deposit->paymentMethod && str_starts_with($deposit->paymentMethod->pay, 'razorpay')) {
                            $raw_dump = $deposit->getAttributes()['auto_res_dump'] ?? null;
                            if (is_string($raw_dump)) {
                                $razorpay_data = json_decode($raw_dump, true);
                            } elseif (is_array($raw_dump)) {
                                $razorpay_data = $raw_dump;
                            }
                        }
                    @endphp
                    ...
                    @if ($razorpay_data && isset($razorpay_data['razorpay_order_id']))
                        <div class="pt-6 border-t border-white/5 relative z-10 mb-6">
                            <h4 class="text-sm font-bold text-white mb-4 uppercase tracking-widest opacity-50">
                                {{ __('Razorpay Transaction Details') }}</h4>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="space-y-1.5 p-4 bg-black/20 rounded-xl border border-white/5">
                                    ...
                                    <div class="flex items-center justify-between gap-2">
                                        <p class="text-white font-mono text-sm truncate">
                                            {{ $razorpay_data['razorpay_order_id'] }}</p>
                                        <button
                                            class="p-1.5 hover:bg-white/10 rounded-lg text-text-secondary transition-colors copy-btn cursor-pointer"
                                ...
                                <div class="space-y-1.5 p-4 bg-black/20 rounded-xl border border-white/5">
                                    ...
                                    <div class="flex items-center justify-between gap-2">
                                        <p class="text-white font-mono text-sm truncate">
                                            {{ $razorpay_data['razorpay_payment_id'] ?? 'Awaiting payment...' }}</p>
                                ...
                                <div class="space-y-1.5 p-4 bg-black/20 rounded-xl border border-white/5 flex flex-col justify-center">
                                    <p class="text-[10px] text-text-secondary uppercase tracking-widest font-bold">
                                        {{ __('Webhook Status') }}</p>
                                    <span class="inline-block mt-1 px-3 py-1 bg-emerald-500/10 text-emerald-500 text-xs rounded-lg border border-emerald-500/20 font-bold uppercase tracking-wider w-max">{{ __('Confirmed') }}</span>
                                </div>
                                @endif
                                ...
                                <div class="space-y-1.5 p-4 bg-black/20 rounded-xl border border-white/5">
                                    ...
                                    <p class="text-white font-bold text-sm uppercase tracking-wider">
                                        {{ $razorpay_data['initial_response']['status'] }}</p>
                                </div>
                                @endif
                            </div>
                        </div>
                    ...
                    @php
                        $stripe_data = null;
                        if ($deposit->paymentMethod && str_contains(strtolower($deposit->paymentMethod->pay), 'stripe')) {
                            $raw_dump = $deposit->getAttributes()['auto_res_dump'] ?? null;
                            if (is_string($raw_dump)) {
                                $stripe_data = json_decode($raw_dump, true);
                            } elseif (is_array($raw_dump)) {
                                $stripe_data = $raw_dump;
                            }
                        }
                    @endphp
                    ...
                    @if ($stripe_data)
                        @php
                            ...
                            $cust_details = $webhook['customer_details'] ?? ($initial['customer_details'] ?? null);
                        @endphp
                        <div class="pt-6 border-t border-white/5 relative z-10 mb-6">
                            <h4 class="text-sm font-bold text-white mb-4 uppercase tracking-widest opacity-50">
                                {{ __('Stripe Transaction Details') }}</h4>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-1.5 p-4 bg-black/20 rounded-xl border border-white/5">
                                    <p class="text-[10px] text-text-secondary uppercase tracking-widest font-bold">
                                        {{ __('Session ID') }}</p>
                                    <div class="flex items-center justify-between gap-2">
                                        <p class="text-white font-mono text-sm truncate">
                                            {{ $stripe_data['stripe_session_id'] ?? ($initial['id'] ?? __('N/A')) }}</p>
                                        <button
                                            class="p-1.5 hover:bg-white/10 rounded-lg text-text-secondary transition-colors copy-btn cursor-pointer"
                                ...
                                <div class="space-y-1.5 p-4 bg-black/20 rounded-xl border border-white/5">
                                    <p class="text-[10px] text-text-secondary uppercase tracking-widest font-bold">
                                        {{ __('Customer Details') }}</p>
                                    <p class="text-white font-bold text-sm">
                                        {{ $cust_details['name'] ?? $deposit->user->fullname }}
                                    </p>
                                    <p class="text-[10px] text-slate-500 font-medium truncate">
                                        {{ $cust_details['email'] ?? $deposit->user->email }}
                                        @if(isset($cust_details['address']['country']))
                                         | {{ $cust_details['address']['country'] }}
                                        @endif
                                    </p>
                                </div>
                                ...
                                <div class="space-y-1.5 p-4 bg-black/20 rounded-xl border border-white/5">
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="px-2 py-0.5 bg-emerald-500/10 text-emerald-500 text-[10px] rounded-lg border border-emerald-500/20 font-bold uppercase tracking-wider">
                                            {{ $webhook['payment_status'] ?? ($initial['payment_status'] ?? __('N/A')) }}
                                        </span>
                                        <span class="px-2 py-0.5 bg-white/5 text-slate-400 text-[10px] rounded-lg border border-white/10 font-bold uppercase tracking-wider">
