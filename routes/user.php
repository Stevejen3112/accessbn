...
use App\Http\Controllers\User\Withdrawal\PaystackController as PaystackWithdrawalController;
...
Route::prefix('user')->middleware(['auth', 'otp.verified', 'user.status', 'user.kyc'])->group(function () {
    ...
    Route::prefix('deposits')->name('deposits.')->group(function () {
        ...
        Route::prefix('new')->name('new.')->group(function () {
            ...
            Route::get('/paystack-card', [PaystackController::class, 'index'])->name('paystack-card');
            Route::get('/paystack-bank', [PaystackController::class, 'index'])->name('paystack-bank');
            Route::get('/paystack-wallet', [PaystackController::class, 'index'])->name('paystack-wallet');
            Route::post('/paystack', [PaystackController::class, 'paystackValidate'])->name('paystack-validate');
            Route::post('/paystack-card', [PaystackController::class, 'paystackValidate'])->name('paystack-card-validate');
            Route::post('/paystack-bank', [PaystackController::class, 'paystackValidate'])->name('paystack-bank-validate');
            Route::post('/paystack-wallet', [PaystackController::class, 'paystackValidate'])->name('paystack-wallet-validate');
            Route::get('/paystack/callback/{transaction_reference}', [PaystackController::class, 'paystackCallback'])->name('paystack-callback');

            // Razorpay
            Route::get('/razorpay', [\App\Http\Controllers\User\Payments\RazorpayController::class, 'index'])->name('razorpay');
            Route::post('/razorpay/initialize', [\App\Http\Controllers\User\Payments\RazorpayController::class, 'razorpayInitialize'])->name('razorpay-initialize');
            Route::post('/razorpay/verify', [\App\Http\Controllers\User\Payments\RazorpayController::class, 'razorpayVerify'])->name('razorpay-verify');
            Route::post('/razorpay/check-status', [\App\Http\Controllers\User\Payments\RazorpayController::class, 'razorpayCheckStatus'])->name('razorpay-check-status');

            // Stripe
            Route::get('/stripe', [\App\Http\Controllers\User\Payments\StripeController::class, 'index'])->name('stripe');
            Route::post('/stripe/initialize', [\App\Http\Controllers\User\Payments\StripeController::class, 'stripeInitialize'])->name('stripe-initialize');
            Route::get('/stripe/success/{transaction_reference}', [\App\Http\Controllers\User\Payments\StripeController::class, 'success'])->name('stripe.success');
            Route::get('/stripe/cancel/{transaction_reference}', [\App\Http\Controllers\User\Payments\StripeController::class, 'cancel'])->name('stripe.cancel');
        });
    });
    ...
    Route::prefix('withdrawals')->name('withdrawals.')->group(function () {
        ...
        // paystack
        Route::get('paystack', [PaystackWithdrawalController::class, 'index'])->name('paystack');
        Route::post('paystack', [PaystackWithdrawalController::class, 'withdraw'])->name('paystack-process');
        Route::post('paystack-resolve', [PaystackWithdrawalController::class, 'resolveAccount'])->name('paystack-resolve');

        // Razorpay
        Route::get('/razorpay', [\App\Http\Controllers\User\Withdrawal\RazorpayController::class, 'index'])->name('razorpay');
        Route::post('/razorpay', [\App\Http\Controllers\User\Withdrawal\RazorpayController::class, 'withdraw'])->name('razorpay-process');
    });
