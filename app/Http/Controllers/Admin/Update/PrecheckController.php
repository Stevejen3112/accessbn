...
class PrecheckController extends Controller
{
    ...
    {
        if (!filter_var(config('site.remote_updates_enabled', false), FILTER_VALIDATE_BOOLEAN)) {
            return response()->json([
                'status' => 'error',
                'message' => __('Remote updater downloads are disabled on this installation.')
            ], 403);
        }
        ...
        try {
            ...
            foreach ($data as $file_path => $content) {
                if (!$this->isAllowedRemoteUpdaterPath($file_path)) {
                    \Log::warning('Updater rejected remote file path: ' . $file_path);
                    continue;
                }
    ...
    private function isAllowedRemoteUpdaterPath(string $file_path): bool
    {
        $path = str_replace('\\', '/', trim($file_path));
